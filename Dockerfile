...
LABEL org.opencontainers.image.title="HK2 Magento PHP 8.1 FPM" \
      org.opencontainers.image.description="PHP 8.1 FPM environment optimized for Magento 2" \
      org.opencontainers.image.source="https://github.com/basantmandal/docker-magento2-php81" \
...
# -----------------------------
# Redis Extension (PHP 8.1 compatible)
# -----------------------------
...
RUN set -eux; \
    ...
    LOADER_FILE="$(find /usr/local/ioncube -name "ioncube_loader_lin_8.1.so" | head -n 1)"; \
    \
    if [ -f "$LOADER_FILE" ]; then \
        cp "$LOADER_FILE" "$EXT_DIR/"; \
        echo "zend_extension=${EXT_DIR}/ioncube_loader_lin_8.1.so" \
